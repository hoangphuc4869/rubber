                        ...
                        <div class="col-12" id="wareModal">
                            ...
                            <select name="slot" required id="slot" class="form-select custom-select-ware w-100">
                                ...
                                @foreach ($warehouses as $item)
                                    <option value="{{$item->id}}" {{$item->batch_id !== null ? 'disabled' : ''}}>{{$item->code}} {!!$item->batch_id !== null ? '(đang chứa hàng)' : ''!!}</option>`
                                @endforeach
                            </select>
    ...
    <table id="datatable" class="ui celled table" style="width:100%">
        ...
        <tbody>
            ...
            <tr id={{$batch->id}} class="{{$batch->exported == 1 ? 'no-select' : ''}}">
                ...
                <td>{!! $batch->warehouse !== null ? '<span class="fw-bold" style="color: #000a8d ">'.$batch->warehouse->code.'</span>' : "<span class='text-danger'>Trống</span>"!!}</td>
                <td>{{ $batch->company->code }}</td>
                <td>{{ $batch->batch_code }}</td>
                <td>{!! $batch->bale_count == 144 ? "<span class='text-dark'>" . $batch->bale_count . "</span>" : "<span class='text-danger'>" . $batch->bale_count . "</span>"!!}</td>
                <td>{!! $batch->checked == 0 ? "<span class='text-danger'>Chưa</span>" : "<span class='text-success'>Đã kiểm nghiệm</span>"!!}</td>
