                ...
                <div class="fs-4 fw-bold">
                    ...
                    <div style="font-size: 14px">Reset lúc
                        ...
                        @if(Gate::allows('admin'))
                            <input type="time" name="time" value="{{$reset->time}}" id="resetTime" class="form-control" style="width: fit-content; display:inline-block">
                        @else
                            <input type="time" value="{{$reset->time}}" id="resetTime" class="form-control" style="width: fit-content; display:inline-block" disabled>
                        @endif
                         mỗi ngày </div> 
            ...
            <form action="{{ route('machining.store') }}" method="POST">
                ...
                <div class="row">
                    <div class="mb-3 col-lg-4">
                        ...
                        <select name="curing_house" class="form-select custom-select w-100 rolling-code-select" id="curing_house" required>
                            <option value="">Chọn nhà ủ</option>
                            @foreach ($houses_containing as $item)
                    ...
                    <div class="mb-3 col-lg-4">
                        ...
                        <select name="rolling_code" class="form-select w-100" required>
                            ...
                            @foreach ($rollings as $item)
                                <option value="{{$item->id}}" data-house="{{$item->house->code}}">{{\Carbon\Carbon::parse($item->date)->format('d/m/Y')}} ({{\Carbon\Carbon::parse($item->time)->format('H:i') .' '.$item->area->code}})</option>   
                            @endforeach
                        </select>   
                    </div>
            ...
            <div class="modal-body">
                <form id="editForm" action="/update-drum-details" method="POST">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6 mb-2">
                            <label class="form-label">Tạp chất loại bỏ</label>
                            <input type="text" name="impurity_removing" id="editImpurity" required class="form-control" placeholder="Nhập tạp chất loại bỏ">
                        </div>
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6 mb-2">
                            <label class="form-label">Bề dày tờ mủ (mm)</label>
                            <input type="text" name="thickness" id="editThickness" required class="form-control" placeholder="Nhập bề dày tờ mủ">
                        </div>
                        <div class="col-md-6 mb-2">
                            <label class="form-label">Trạng thái cốm</label>
                            <input type="text" required name="trang_thai_com" id="editTrangThaiCom" class="form-control" placeholder="Nhập trạng thái cốm">
                        </div>
                    </div>
                    <input type="hidden" id="drumId" name="drum_id">
                    <div class="modal-footer p-0">
                        <button type="submit" class="btn btn-success" id="saveChangesBtn" form="editForm">Lưu thay đổi</button>
                    </div>  
                </form>
            </div>
        </div>
