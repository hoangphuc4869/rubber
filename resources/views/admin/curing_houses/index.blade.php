...
<div class="row">
    <div class="col-lg-4">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Tạo nhà ủ</h5>
            </div>
            <div class="card-body">
                @include('partials.errors')
                
                <form action="{{ route('curing_houses.store') }}" method="POST">
                    @csrf
                    <div class="mb-3">
    ...
    <div class="col-lg-8">
        ...
        <table id="datalist" class="ui celled table" style="width:100%">
            ...
            <tbody>
                @foreach ($curing_houses as $index => $curing_houses)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ $curing_houses->code }}</td>
                    <td>{{ $curing_houses->name }}</td>
                    <td>{{ $curing_houses->curing_area ? $curing_houses->curing_area->code : '' }}</td>
                    <td>
                       <div class="custom d-flex gap-1">
                         <a href="{{route('curing_houses.edit', [$curing_houses->id])}}">
                        ...
                        <form action="{{route('curing_houses.destroy', [$curing_houses->id])}}" method="POST" onsubmit="return confirmDelete();">
                            @csrf
                            @method('DELETE')
